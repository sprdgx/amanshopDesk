...
export default function Orders() {
  ...
  useEffect(() => {
    const checkForNewOrders = async () => {
      try {
        const res = await userRequest.get("/orders/new");
        if (res.data.length > 0 && !res.data.every(order => order.confirmed)) {
          showNotification();
        }
      } catch (error) {
    ...
    // Check for new orders initially
    checkForNewOrders();

    const intervalId = setInterval(checkForNewOrders, 900000);
  ...
  const showNotification = () => {
    if ('Notification' in window && Notification.permission === 'granted') {
      new Notification('New Order', {
        body: 'There are new unconfirmed orders!',
      });
    } else {
