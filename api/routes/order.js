...
// GET new orders that are not confirmed
router.get("/new", verifyTokenAndAdmin, async (req, res) => {
  try {
    // Find orders that are not confirmed yet
    const newOrders = await Order.find({ confirmed: false });

    // Send the new orders as response
